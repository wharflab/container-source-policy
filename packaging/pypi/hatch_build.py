...
def get_platform_info():
    target_platform = os.environ.get('CSP_TARGET_PLATFORM')
    target_arch = os.environ.get('CSP_TARGET_ARCH')
...
class CustomBuildHook(BuildHookInterface):
    ...
    def _prune_binaries(self):
        ...
        bin_dir = Path(self.root) / "container_source_policy" / "bin"
        if not bin_dir.is_dir():
            raise RuntimeError(f"Bin directory not found: {bin_dir}")

        target_dir_name = f"container-source-policy-{self.target_platform}-{self.target_arch}"
        ...
        binaries = list(target_dir.glob("container-source-policy*"))
        if not binaries:
            raise FileNotFoundError(
                f"No container-source-policy binary found under {target_dir}."
            )

        self._temp_dir = Path(tempfile.mkdtemp(prefix="csp-bin-backup-"))
